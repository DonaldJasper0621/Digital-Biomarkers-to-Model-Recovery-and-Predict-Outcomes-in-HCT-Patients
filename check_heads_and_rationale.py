...
# Make sure it contains four coloums
...
if set(["ASLEEP_MIN", "INBED_VALUE", "MOOD"]).issubset(same_day.columns):
    with np.errstate(divide="ignore", invalid="ignore"):  #errstate is a function to prevent warning pop out when divided by 0 
        sleep_eff = same_day["ASLEEP_MIN"] / same_day["INBED_VALUE"]
    corr_sleep_eff_mood = (
        pd.concat([sleep_eff.rename("sleep_efficiency"), same_day["MOOD"]], axis=1) # concat horizontally
        .corr()
        .iloc[0, 1]
    )
else:
